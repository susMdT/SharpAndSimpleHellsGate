...
namespace SharpAndSimpleHellsGate
{
    ...
    /// Used to extract Structs and find system calls.
    /// </summary>
    public class SystemModule : MemoryUtil
    {
        ...
        /// </summary>
        public Structs.IMAGE_DOS_HEADER ModuleDOSHeader { get; private set; }

        /// <summary>
        /// IMAGE_NT_HEADERS64 structure of the NTDLL module.
        /// </summary>
        public Structs.IMAGE_NT_HEADERS64 ModuleNTHeaders { get; private set; }

        /// <summary>
        /// IMAGE_SECTION_HEADER structure from the NTDLL module.
        /// </summary>
        public List<Structs.IMAGE_SECTION_HEADER> ModuleSectionHeaders { get; private set; }

        /// <summary>
        /// IMAGE_EXPORT_DIRECTORY structure from the NTDLL module.
        /// </summary>
        public Structs.IMAGE_EXPORT_DIRECTORY ModuleExportDirectory { get; private set; }
        ...
        public SystemModule(string name) : base()
        {
            ...
            this.ModuleSectionHeaders = new List<Structs.IMAGE_SECTION_HEADER>() { };
        ...
        public bool LoadModule()
        {
            ...
            {
                Console.WriteLine("Module name not provided");
                return false;
            }

            if (!File.Exists(this.ModulePath))
            {
                Console.WriteLine($"Unable to find module: {this.ModuleName}");
                return false;
            }
            
            byte[] ModuleBlob = File.ReadAllBytes(this.ModulePath);
            if (ModuleBlob.Length == 0x00)
            {
                Console.WriteLine($"Empty module content: {this.ModuleName}");
                return false;
            }
        ...
        {
            if (this.GetModuleDOSHeader(true).Equals(default(Structs.IMAGE_DOS_HEADER)))
                return false;

            if (this.GetModuleNTHeaders(true).Equals(default(Structs.IMAGE_NT_HEADERS64)))
                return false;

            if (this.GetModuleSectionHeaders(true).Count != this.ModuleNTHeaders.FileHeader.NumberOfSections)
                return false;

            if (this.GetModuleExportDirectory(true).Equals(default(Structs.IMAGE_EXPORT_DIRECTORY)))
                return false;
        ...
        public Structs.IMAGE_DOS_HEADER GetModuleDOSHeader(bool ReloadCache = false)
        {
            if (!this.ModuleDOSHeader.Equals(default(Structs.IMAGE_DOS_HEADER)) && !ReloadCache)
            ...
            this.ModuleDOSHeader = base.GetStructureFromBlob<Structs.IMAGE_DOS_HEADER>(0);
        ...
        /// <param name="ReloadCache">Whether the data has to re-processed if not already cached.</param>
        /// <returns>The IMAGE_NT_HEADERS64 structure of the module.</returns>
        public Structs.IMAGE_NT_HEADERS64 GetModuleNTHeaders(bool ReloadCache = false)
        {
            if (!this.ModuleNTHeaders.Equals(default(Structs.IMAGE_NT_HEADERS64)) && !ReloadCache)
            ...
            if (this.ModuleDOSHeader.Equals(default(Structs.IMAGE_DOS_HEADER)))
                this.GetModuleDOSHeader();

            this.ModuleNTHeaders = base.GetStructureFromBlob<Structs.IMAGE_NT_HEADERS64>(this.ModuleDOSHeader.e_lfanew);
        ...
        /// </summary>
        /// <param name="ReloadCache">Whether the data has to re-processed if not already cached.</param>
        /// <returns>The list of _IMAGE_SECTION_HEADER Structs.</returns>
        public List<Structs.IMAGE_SECTION_HEADER> GetModuleSectionHeaders(bool ReloadCache = false)
        {
            ...
            if (this.ModuleNTHeaders.Equals(default(Structs.IMAGE_NT_HEADERS64)) || this.ModuleNTHeaders.FileHeader.Equals(default(Structs.IMAGE_FILE_HEADER)))
            ...
            for (Int16 cx = 0; cx < this.ModuleNTHeaders.FileHeader.NumberOfSections; cx++)
            {
                ...
                Structs.IMAGE_SECTION_HEADER ImageSection = base.GetStructureFromBlob<Structs.IMAGE_SECTION_HEADER>(iSectionOffset);
                if (!ImageSection.Equals(default(Structs.IMAGE_SECTION_HEADER)))
        ...
        public Structs.IMAGE_SECTION_HEADER GetModuleSectionHeaderByName(string name)
        {
            if (name.Length > 8)
            {
        ...
        public Structs.IMAGE_EXPORT_DIRECTORY GetModuleExportDirectory(bool ReloadCache = false)
        {
            if (!this.ModuleExportDirectory.Equals(default(Structs.IMAGE_EXPORT_DIRECTORY)) && !ReloadCache)
            ...
            if (this.ModuleNTHeaders.Equals(default(Structs.IMAGE_NT_HEADERS64)))
                this.GetModuleNTHeaders();
            ...
            this.ModuleExportDirectory = base.GetStructureFromBlob<Structs.IMAGE_EXPORT_DIRECTORY>(this.ModuleExportDirectoryOffset);
            if (this.ModuleExportDirectory.Equals(default(Structs.IMAGE_EXPORT_DIRECTORY)))
            {
                Console.WriteLine("Invalid export address table (EAT).");
                return default;
            }
        ...
        /// <returns></returns>
        public short GetSyscallId(string funcName)
        {
            ...
            for (Int32 cx = 0; cx < this.ModuleExportDirectory.NumberOfNames; cx++)
            {
                ...
                if (FunctionName.Equals(funcName))
                {
                    ...
                    byte[] opcode = base.GetFunctionOpCode(this.ConvertRvaToOffset(PtrFunctionAdddress));

                    if (opcode[3] == 0xb8 && opcode[18] == 0x0f && opcode[19] == 0x05)
                    {
                        return (Int16)(((byte)opcode[5] << 4) | (byte)opcode[4]);
                    }
                }
            }
        ...
        private Int64 GetModuleSectionOffset(Int16 cx)
            ...
            + Marshal.SizeOf(typeof(Structs.IMAGE_FILE_HEADER))
            + this.ModuleNTHeaders.FileHeader.SizeOfOptionalHeader
            + sizeof(Int32) // sizeof(DWORD)
            + (Marshal.SizeOf(typeof(Structs.IMAGE_SECTION_HEADER)) * cx);
        ...
        private Int64 ConvertRvaToOffset(Int64 rva, Structs.IMAGE_SECTION_HEADER SectionHeader) => rva - SectionHeader.VirtualAddress + SectionHeader.PointerToRawData;
        ...
        /// <returns>The _IMAGE_SECTION_HEADER structure</returns>
        private Structs.IMAGE_SECTION_HEADER GetSectionByRVA(Int64 rva) => this.ModuleSectionHeaders.Where(x => rva > x.VirtualAddress && rva <= x.VirtualAddress + x.SizeOfRawData).FirstOrDefault();
    }
}
